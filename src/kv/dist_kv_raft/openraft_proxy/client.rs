...
use super::{
    store::{ExampleRequest, ExampleResponse},
    ExampleTypeConfig,
};
...
impl ExampleClient {
    ...
    pub async fn write(
        ...
        RPCError<NodeID, BasicNode, ClientWriteError<NodeID, BasicNode>>,
    > {
    ...
    pub async fn read(
        &self,
        req: &String,
    ) -> Result<String, RPCError<NodeID, BasicNode, Infallible>> {
    ...
    pub async fn consistent_read(
        &self,
        req: &String,
    ) -> Result<String, RPCError<NodeID, BasicNode, CheckIsLeaderError<NodeID, BasicNode>>> {
    ...
    pub async fn init(
        &self,
    ) -> Result<(), RPCError<NodeID, BasicNode, InitializeError<NodeID, BasicNode>>> {
    ...
    //     RPCError<NodeID, BasicNode, AddLearnerError<NodeID, BasicNode>>,
    // > {
    ...
    pub async fn metrics(
        &self,
    ) -> Result<RaftMetrics<NodeID, BasicNode>, RPCError<NodeID, BasicNode, Infallible>> {
    ...
    where
        ...
        Err: std::error::Error
            + Serialize
            + DeserializeOwned
            + TryInto<ForwardToLeader<NodeID, BasicNode>>
            + Clone,
    {
        ...
        loop {
            let res: Result<Resp, RPCError<NodeID, BasicNode, Err>> =
            ...
            if let RPCError::RemoteError(remote_err) = &rpc_err {
                let forward_err_res =
                    <Err as TryInto<ForwardToLeader<NodeID, BasicNode>>>::try_into(
