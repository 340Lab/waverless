...
pub fn add_routers(mut router:Router)->Router
{
    ...
    async fn add_service(Json(req):Json<AddServiceReq>)-> (StatusCode, Json<Value>){
        (StatusCode::OK, Json(ApiHandlerImpl.handle_add_service(req).await.serialize()))
    }
    router=router
        .route("/add_service", post(add_service));
                             
    async fn delete_service(Json(req):Json<DeleteServiceReq>)-> (StatusCode, Json<Value>){
        (StatusCode::OK, Json(ApiHandlerImpl.handle_delete_service(req).await.serialize()))
    }
    router=router
        .route("/delete_service", post(delete_service));
                             
    async fn get_service_list()-> (StatusCode, Json<Value>){
        (StatusCode::OK, Json(ApiHandlerImpl.handle_get_service_list().await.serialize()))
    }
    router=router
        .route("/get_service_list", post(get_service_list));
                             
    async fn run_service_action(Json(req):Json<RunServiceActionReq>)-> (StatusCode, Json<Value>){
        (StatusCode::OK, Json(ApiHandlerImpl.handle_run_service_action(req).await.serialize()))
    }
    router=router
        .route("/run_service_action", post(run_service_action));
