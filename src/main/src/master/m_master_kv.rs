...
// use crate::{
//     general::{
//         m_appmeta_manager::{fn_event::EventTriggerInfo, AppMetaManager},
//         m_data_general::{DataGeneral, DATA_UID_PREFIX_APP_META, DATA_UID_PREFIX_FN_KV},
...
// logical_module_view_impl!(MasterKvView, data_general, DataGeneral);
...
//         let mut kvs = vec![];
//         let key = get.range.unwrap().start;
//         if let Some(v) = data_general
//             .get_data_item(DATA_UID_PREFIX_FN_KV.as_bytes().to_owned().extend(key), 0)
//             .await
//         {
